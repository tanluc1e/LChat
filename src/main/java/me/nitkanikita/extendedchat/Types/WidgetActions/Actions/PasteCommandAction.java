...
public class PasteCommandAction implements IWidgetAction {
   ...
   public PasteCommandAction(String command) {
      this.cmd = command;
   }

   public TextComponent Register(TextComponent source) {
      source.setClickEvent(new ClickEvent(Action.SUGGEST_COMMAND, this.cmd));
      return source;
   }
   ...
   public TextComponent ParseAndRegister(TextComponent source, String message, Player player, String id) {
      ...
      component.setClickEvent(new ClickEvent(Action.SUGGEST_COMMAND, txt));
      return component;
   }
}
